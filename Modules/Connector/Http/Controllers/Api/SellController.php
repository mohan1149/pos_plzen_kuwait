...
class SellController extends ApiController
{
     ...
     * @queryParam res_waiter_id id of the service staff assigned with the sale
    ...
    public function index()
    {
        ...
        $filters = request()->only(['location_id', 'contact_id', 'payment_status', 'start_date', 'end_date', 'user_id', 'res_waiter_id', 'only_subscriptions', 'per_page', 'shipping_status', 'order_by_date', 'source', 'status']);
        ...
        if (!empty($filters['status'])) {
            ...
            } else if ($filters['status'] == 'quotation') {
                $query->where('transactions.sub_status', 'quotation');
            } else if ($filters['status'] == 'proforma') {
        ...
        if (!empty($filters['res_waiter_id'])) {
            $query->where('transactions.res_waiter_id', $filters['res_waiter_id']);
        }
    ...
    * @bodyParam sells.*.res_waiter_id int id of the service staff assigned to the sale
    ...
    * @bodyParam res_waiter_id int id of the service staff assigned to the sale
    ...
    */
    ...
    private function __formatSellData($data, $transaction = null)
    {
        ...
        $formated_data = [
            ...
            'res_waiter_id' => $this->__getValue('res_waiter_id', $data, $transaction, null, 'res_waiter_id'),
        ...
        if (!empty($data['products'])) {
            foreach ($data['products'] as $product_data) {
                ...
                $formated_sell_line = [
                    ...
                    'res_service_staff_id' => $this->__getValue('res_service_staff_id', $product_data, $sell_line, null, 'res_service_staff_id')
                ];
                if (!empty($sell_line)) {
